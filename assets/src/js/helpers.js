...
function woogosendToggleButtons(args) {
  var data = woogosendGetButtons(args);
  $('#woogosend-buttons').remove();
  $('#btn-ok').hide().after(wp.template('woogosend-buttons')(data));
}

function woogosendGetButtons(args) {
  var buttonLabels = woogosend_backend.i18n.buttons;
...
function woogosendI18n(path) {
  if (typeof path === 'string') {
    path = path.split('.');
  }

  return _.property(path)(woogosend_backend.i18n);
}

function woogosendError(path) {
  if (typeof path === 'string') {
    path = path.split('.');
  }

  return _.property(path)(woogosend_backend.i18n.errors);
}
