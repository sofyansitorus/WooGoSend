(function ($) {
	...
	var wooGoSend = {
		...
		inputApiKeyId: 'woocommerce_woogosend_gmaps_api_key',
		...
		init: function () {
			...
			$(document).on('click', '.wc-shipping-zone-method-settings', function () {
				if ($(this).closest('tr').find('.wc-shipping-zone-method-type').text() === woogosend_params.method_title) {
		...
		_loadScript: function (apiKey) {
			...
			try {
				var scriptSrc = 'https://maps.googleapis.com/maps/api/js?key=' + apiKey + '&libraries=geometry,places';
		...
		_buildMap: function () {
			...
			if (!$('#' + self.mapCanvasId).length) {
				$('#' + self.mapWrapperId).append(tmplMapCanvas({
			...
			if (!$('#' + self.mapSearchId).length) {
				$('#' + self.mapWrapperId).append(tmplMapSearch({
