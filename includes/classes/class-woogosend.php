...
class WooGoSend {
	...
	private function __construct() {
		...
		add_action( 'wp_ajax_woogosend_validate_api_key_server', array( $this, 'validate_api_key_server' ) );
	...
	public function plugin_action_links( $links ) {
		...
		foreach ( WC_Shipping_Zones::get_zones() as $zone ) {
			...
			foreach ( $zone['shipping_methods'] as $zone_shipping_method ) {
				if ( $zone_shipping_method instanceof WooGoSend_Shipping_Method ) {
	...
	public function enqueue_backend_assets( $hook = null ) {
		...
		$is_dev_env = woogosend_is_dev_env();
	...
	public function register_shipping_method( $methods ) {
		if ( class_exists( 'WooGoSend_Shipping_Method' ) ) {
			$methods[ WOOGOSEND_METHOD_ID ] = 'WooGoSend_Shipping_Method';
		}
	...
	public function after_shipping_calculator() {
		...
		foreach ( $fields as $key => $field ) {
			...
			?>
			<input type="hidden" id="woogosend-calc-shipping-field-value-<?php echo esc_attr( $key ); ?>" value="<?php echo esc_attr( $field['value'] ); ?>" data-field="<?php echo esc_attr( wp_json_encode( $field['data'] ) ); ?>" />
			<?php
		}

		?>
		<script type="text/template" id="tmpl-woogosend-calc-shipping-custom-field">
	...
	public function inject_cart_shipping_packages( $packages ) {
		...
		foreach ( array_keys( $address_fields ) as $field_key ) {
			$address_fields[ $field_key ] = woogosend_calc_shipping_field_value( 'calc_shipping_' . $field_key );
		}
